...
5. Designate `builder` as the only tracked-file writer. Keep the Lead and critics read-only on tracked/project files. The Lead may write only its task-scoped ignored control records; `verification_runner` writes only its verification evidence subdirectory; one Lead-recorded owner writes UI/browser evidence. Serialize these writes and stop `BLOCKED` on ambiguous ownership or tracked-state mutation. Focused builder tests remain provisional; `verification_runner` independently reruns selected and final checks.
