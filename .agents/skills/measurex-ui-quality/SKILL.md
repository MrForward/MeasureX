...
4. The Lead arranges a local app. Use Browser or Chrome tooling when available and permitted; record current and revised screenshots with route, state, viewport, timestamp, artifact path, and evidence owner. Browser unavailable is `NOT RUN`, distinct from the human taste gate. Never use image generation as a substitute for interface design.
