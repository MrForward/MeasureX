...
2. Record a [discovery brief](../../../docs/agent-operations/templates/discovery-brief.md). The Lead may persist the brief and later requirements lock only as Lead-owned, task-scoped ignored control artifacts while remaining read-only on tracked files. Use `product_strategist` for cited customer, problem, alternatives, and hypothesis evidence. Public research requires separate Lead authorization and an available read-only research tool; the repository shell and build workflow stay network-disabled.
