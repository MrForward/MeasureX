...
2. Have read-only `qa` map acceptance criteria, test design, manual checks, and external-state needs. Have `verification_runner` alone execute documented deterministic commands and write only its ignored verification evidence subdirectory. The Lead owns `run.json` and control manifests; serialize artifact updates and never touch tracked files.
